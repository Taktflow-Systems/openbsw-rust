...
`libs/bsw/estd/include/estd`. Every public upstream header is classified below.
...
| `big_endian.h` | ported | `bsw_estd::big_endian` tests |
| `bitset.h` | ported | `bsw_estd::Bitset` tests |
| `constructor.h` | native replacement: `MaybeUninit`, `Default`, placement through pool APIs | Miri container suite |
| `forward_list.h` | ported with direct node pointers and explicit `clear` teardown | `forward_list` tests and Miri |
...
| `object_pool.h` | ported; zero-sized types are rejected | `object_pool` tests and Miri |
| `optional.h` | native replacement: `Option<T>` | compiler |
| `ordered_map.h` | ported | `OrderedMap` tests and Miri |
...
| `vector.h` | ported as `FixedVec<T, N>` | vector tests, Miri, fuzz |
